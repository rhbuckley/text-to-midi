...
if __name__ == "__main__":
    ...
    assert train_size > 0 and val_size > 0, f"Dataset split resulted in zero samples for train ({train_size}) or validation ({val_size}). Check validation_split ({CONFIG['validation_split']}) and dataset size ({total_size})."
    ...
    n_update_steps_per_epoch = math.ceil(len(train_dataloader) / CONFIG["gradient_accumulation_steps"])
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            if run:
                ...
                run.log({
                    "train/step_loss": loss.item() * CONFIG["gradient_accumulation_steps"],
                    "train/learning_rate": current_lr,
                    "global_step": global_step
                })

            # Update progress bar description
            progress_bar.set_postfix(loss=f"{loss.item() * CONFIG['gradient_accumulation_steps']:.4f}", lr=f"{scheduler.get_last_lr()[0]:.2e}")
        ...
        if run:
            try:
                ...
                if avg_val_loss < best_val_loss:
                    ...
                    run.link_artifact(artifact, f"{CONFIG['wandb_project']}/model_best", aliases=["best"])
    ...
    if run:
        try:
            final_artifact = wandb.Artifact("final_model", type="model", metadata={"epochs": CONFIG["epochs"], "final_val_loss": avg_val_loss})
            final_artifact.add_dir(CONFIG["model_save_path"] + "/epoch_" + str(CONFIG["epochs"] + 1))
